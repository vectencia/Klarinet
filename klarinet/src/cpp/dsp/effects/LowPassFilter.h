...
/**
 * @brief Parameter IDs for the LowPassFilter effect.
 */
namespace LPFParams {
    /// @brief Cutoff frequency in Hertz. ID = 0.
    ///
    /// Frequencies above this value are progressively attenuated.
    /// The filter has a -3 dB point at the cutoff frequency.
    /// Range: 20.0 to 20000.0 Hz.
    /// Default: 20000.0 Hz (fully open, no filtering).
    /// Units: Hertz (Hz).
...
/**
 * @brief Single second-order (biquad) low-pass filter.
 *
 * Attenuates frequencies above the cutoff frequency at a rate of
 * -12 dB/octave (second-order roll-off). The resonance parameter
 * controls whether the response is flat (Butterworth at Q = 0.707)
 * or has a peak at the cutoff frequency.
 *
 * @par DSP Algorithm
 * Uses a biquad filter configured as BiquadType::LowPass. The biquad
 * implements the standard second-order IIR transfer function:
 *
...
public:
    /** @brief Constructs a LowPassFilter with cutoff at 20 kHz (fully open). */
    LowPassFilter();
    ...
    /**
     * @brief Processes the audio buffer through the low-pass biquad filter.
     *
     ...
     * next process() call.
     *
     * @param paramId  Parameter ID (see LPFParams).
...
private:
    /// @brief Cutoff frequency in Hz. Default: 20000.0 Hz (fully open).
