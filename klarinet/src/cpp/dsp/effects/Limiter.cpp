...
void Limiter::process(float* buffer, int32_t numFrames, int32_t channelCount) {
    ...
    for (int32_t frame = 0; frame < numFrames; ++frame) {
        ...
        // Step 2: Track envelope (smoothed by near-instant attack / configurable release).
        float envLevel = envFollower_.process(peak);

        // Step 3: Brick-wall limiting - if envelope exceeds threshold, reduce gain
        // so that output = threshold. gain = threshold / envelope.
        float gainLinear = 1.0f;
        if (envLevel > threshLinear) {
            gainLinear = threshLinear / envLevel;
        }

        // Step 4: Apply gain reduction to all channels in this frame.
        for (int32_t ch = 0; ch < channelCount; ++ch) {
